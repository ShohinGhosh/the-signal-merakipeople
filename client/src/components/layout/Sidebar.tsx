...
const NAV_ITEMS = [
  ...
  { path: '/signal-feed', label: 'Signal Feed', icon: Radio, requiresStrategy: true },
  { path: '/calendar', label: 'Calendar', icon: Calendar, requiresStrategy: true },
  { path: '/brain', label: 'Brain', icon: Brain, requiresStrategy: true },
  { path: '/analytics', label: 'Analytics', icon: BarChart3, requiresStrategy: true },
  { path: '/pipeline', label: 'Pipeline', icon: Link2, requiresStrategy: true },
...
export default function Sidebar({ onAddSignal }: { onAddSignal: () => void }) {
  ...
  return (
    <aside
      className={`fixed left-0 top-0 h-full bg-brand-indigo text-white flex flex-col transition-all duration-300 z-50 ${
        collapsed ? 'w-16' : 'w-60'
      }`}
    >
      {/* Header */}
      <div className="p-4 border-b border-white/10">
        {!collapsed && (
          <div>
            ...
            <h2 className="text-lg font-bold">The Signal</h2>
          </div>
        )}
      ...
      <button
        ...
        className="absolute -right-3 top-1/2 w-6 h-6 bg-brand-indigo border border-white/20 rounded-full flex items-center justify-center text-white/50 hover:text-white"
      >
