...
import { Radio, ChevronLeft, ChevronRight, Link2, Check, ArrowRight, Calendar } from 'lucide-react';
...
const ROUTING_BADGES: Record<string, { label: string; color: string }> = {
  ...
  archive: { label: 'Archive', color: 'bg-gray-50 text-gray-600 border-gray-200' },
};
...
export default function SignalFeedPage() {
  ...
  const loadEntries = useCallback(async () => {
    try {
      ...
      end.setDate(weekStart.getDate() + 13); // Load 2 weeks for context
  ...
  // Also collect "overflow" entries from the second week
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2 text-slate-900">
            <Radio size={24} className="text-brand-coral" />
            Signal Diary
          </h1>
          <p className="text-sm text-slate-400 mt-1">Capture ideas, URLs, observations — fuel for your content calendar</p>
        </div>
        <div className="flex gap-2">
          <select
            value={filter.author || ''}
            onChange={(e) => setFilter((f) => ({ ...f, author: e.target.value || undefined }))}
            className="bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-sm text-slate-900"
          >
      ...
      {/* Quick Add Form */}
      <div className={`bg-white border rounded-xl p-4 mb-8 transition-colors shadow-sm ${
        successFlash ? 'border-green-500/50 bg-green-50' : 'border-slate-200/60'
      }`}>
        <textarea
          value={quickText}
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-full text-xs transition-colors ${
                quickTags.includes(tag.key)
                  ? 'bg-brand-coral/20 text-brand-coral border border-brand-coral/40'
                  : 'bg-slate-100 text-slate-400 border border-transparent hover:text-slate-600 hover:bg-slate-200'
              }`}
            >
              {tag.emoji} {tag.label}
            </button>
        ...
        <div className="flex items-center gap-2">
          {showUrlInput ? (
            <input
              ...
              placeholder="https://..."
              className="flex-1 bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-sm text-slate-900 placeholder-slate-300 outline-none focus:border-brand-coral/50 focus:ring-1 focus:ring-brand-coral/20"
            />
          ) : (
            <button
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-3">
          <h2 className="text-lg font-semibold text-slate-900">{formatWeekLabel(weekStart)}</h2>
          <span className="text-xs text-slate-300">
            {weekStart.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} – {weekEnd.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
          </span>
          {thisWeekCount > 0 && (
            <span className="px-2 py-0.5 bg-brand-coral/20 text-brand-coral text-xs rounded-full font-medium">
              {thisWeekCount} signals
            </span>
          )}
        </div>
        <div className="flex items-center gap-1">
          {formatWeekLabel(weekStart) !== 'This Week' && (
            <button
              ...
              className="text-xs text-slate-400 hover:text-slate-600 mr-2 transition-colors"
            >
              Today
            </button>
          )}
            ...
            return (
              <div key={dateKey}>
                ...
                <div className={`flex items-center gap-2 py-2 px-1 ${isToday ? 'text-brand-coral' : 'text-slate-400'}`}>
                  ...
                  {isToday && <span className="text-[10px] bg-brand-coral/20 px-1.5 py-0.5 rounded-full">today</span>}
                  {dayEntries.length > 0 && (
...
function EntryCard({ entry, onConfirm }: { entry: SignalFeedEntry; onConfirm: (id: string) => void }) {
  ...
  return (
    <div className={`bg-white border border-slate-200/60 rounded-lg p-3 hover:shadow-md transition-shadow shadow-sm ${borderClass}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          {/* Author + time */}
          <div className="flex items-center gap-2 mb-1.5">
            <div className="w-5 h-5 rounded-full bg-brand-coral/20 flex items-center justify-center text-brand-coral text-[10px] font-medium capitalize">
              {entry.author[0]}
            </div>
          ...
          {entry.tags?.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              {entry.tags.map((tag) => {
                ...
                return (
                  <span key={tag} className="px-1.5 py-0.5 bg-slate-100 rounded text-[10px] text-slate-400">
        ...
        <div className="flex flex-col items-end gap-1.5 shrink-0">
          ...
          {entry.routing && (
            <span className={`px-2 py-0.5 rounded text-[10px] font-medium border ${ROUTING_BADGES[entry.routing]?.color || ''}`}>
